...
- `PROVEN` 中立且可用的 domain 樣本：**173**（母體 3.10%）
- `PROVEN` production：**28.7%**（maturity 有效樣本 171）
- `PROVEN` agent target：**1.8%**（target 有效樣本 170）
...
|---|---:|---:|
| 配置 (`configure`) | 5 / 28.5 | 0.18x |
| 調度 (`orchestrate`) | 5 / 26.1 | 0.19x |
| 分析 (`analyze`) | 6 / 21.0 | 0.29x |
